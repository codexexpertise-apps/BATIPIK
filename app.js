...
let artisanLogoData = '';
const draftNumeroState = { carrelage: null, placo: null };
...
function remplirFormParametres() {
  ...
  setValue('artisan-statut', s.artisanStatut);
...
function sauverParametres(e) {
  ...
  const settings = {
    ...
    artisanStatut: getInput('artisan-statut'),
...
function getDraftIdentity(module) {
  if (module === 'carrelage') {
    return [module, getInput('client-carrelage'), getInput('adresse-carrelage'), getInput('email-carrelage'), getInput('telephone-carrelage')].join('|').toLowerCase();
  }
  return [module, getInput('client-placo'), getInput('adresse-placo'), getInput('email-placo'), getInput('telephone-placo')].join('|').toLowerCase();
}
...
function getPrixDetail(module, surface, prixGlobal) {
  ...
  return {
    ...
    lignes: module === 'carrelage'
      ? [
          { libelle: 'Carrelage fourni', quantite: surface, unite: 'm²', puHT: materiau, montantHT: surface * materiau },
          { libelle: 'Pose du carrelage', quantite: surface, unite: 'm²', puHT: pose, montantHT: surface * pose },
...
function calculerCarreleur(e) {
  ...
  const chantier = creerChantierCommun({
    ...
    numeroDevis, surface, tauxTva, remisePourcent, labelSup, montantSup, prestationsComplementaires, finance, prixDetaille: prixDetail.active, photo: photoCache.carrelage,
...
function calculerPlaquiste(e) {
  ...
  const chantier = creerChantierCommun({
    ...
    typePlacoLabel: typePlaqueLabel, numeroDevis, surface, tauxTva, remisePourcent, labelSup, montantSup, prestationsComplementaires, finance, prixDetaille: prixDetail.active, photo: photoCache.placo,
...
function creerChantierCommun({ metier, client, adresse, dateSouhaitee, email, telephone, details, numeroDevis, surface, format, typeChantier, typePlacoLabel, tauxTva, remisePourcent, labelSup, montantSup, prestationsComplementaires = [], finance, colisage, prixDetaille, photo }) {
  return { numeroDevis: numeroDevis || genererNumeroDevis(), metier, client: client || 'Client non renseigné', adresse: adresse || '', dateSouhaitee: dateSouhaitee || '', email: email || '', telephone: telephone || '', statut: 'en_attente', details, surface, format: format || '', typeChantier: typeChantier || '', typePlacoLabel: typePlacoLabel || '',
    ...
    ligneSup: montantSup > 0 ? { label: labelSup || 'Travaux complémentaires', montant: euro(montantSup) } : null, prestationsComplementaires: prestationsComplementaires.map((item) => ({ libelle: item.libelle, montant: euro(item.montant) })), remiseLabel: remisePourcent > 0 ? `Remise de ${remisePourcent}% incluse` : null,
...
function lignesExportDevis(chantier) {
  ...
  if (chantier.ligneSup) lignes.push({ libelle: chantier.ligneSup.label, quantite: 1, unite: 'forfait', puHT: chantier.ligneSup.montant, totalHT: chantier.ligneSup.montant });
  return lignes;
}
